...
int			numvktextures;
int			base_textureid;		// vktextures[i] = base_textureid+i
...
int		vk_tex_alpha_format = 0;

int		vk_filter_min = VK_FILTER_LINEAR;
int		vk_filter_max = VK_FILTER_LINEAR;
int     vk_filter_mipmap = VK_SAMPLER_MIPMAP_MODE_LINEAR;

typedef struct
{
	char *name;
    int	maximize, minimize;
    int mipmapmode;
} vkmode_t;
...
vktmode_t vk_alpha_modes[] = {
	{"VK_RGBA", VK_FORMAT_R8G8B8A8_UNORM },
};

#define NUM_VK_ALPHA_MODES (sizeof(vk_alpha_modes) / sizeof (vktmode_t))
...
void Vk_TextureMode( char *string )
{
	...
	vk_filter_min = modes[i].minimize;
	vk_filter_max = modes[i].maximize;

	// TODO: change active Vulkan sampler
}
...
void Vk_TextureAlphaMode( char *string )
{
	...
	for (i=0 ; i< NUM_VK_ALPHA_MODES ; i++)
	{
		if ( !Q_stricmp( vk_alpha_modes[i].name, string ) )
			break;
	}

	if (i == NUM_VK_ALPHA_MODES)
	{
		ri.Con_Printf (PRINT_ALL, "bad alpha texture mode name\n");
		return;
	}

	vk_tex_alpha_format = vk_alpha_modes[i].mode;
}
...
qboolean Vk_Upload32 (unsigned *data, int width, int height,  qboolean mipmap)
{
    ...
    // TODO: Vulkan texture upload through staiging buffer
...
void	Vk_InitImages (void)
{
	...
	if (false)
	{
		ri.FS_LoadFile( "pics/16to8.dat", &vk_state.d_16to8table );
		if ( !vk_state.d_16to8table )
